...
class LoginForm(QWidget):
    ...
    def init_ui(self):
        ...
        # Add name field only for student and teacher roles
    ...
    def attempt_login(self):
        # Get name only if the field exists (for student and teacher)
        name = self.name_input.text().strip() if self.role in ["student", "teacher"] else self.username_input.text().strip()
        username = self.username_input.text().strip()
        password = self.password_input.text().strip()
        
        if self.role in ["student", "teacher"] and not name:
        ...
        for user in users:
            if user["username"] == username and user["password"] == password:
                self.on_login(self.role, name, username)  # Pass name and username
                return
